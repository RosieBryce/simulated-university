...
def visualize_curriculum_assessment_data(csv_file="stonegrove_curriculum_assessment_marks.csv"):
    """Create visualizations for the curriculum-based assessment data."""
    
    print("📊 Creating curriculum-based assessment visualizations...")
    ...
    plt.tight_layout()
    plt.savefig('stonegrove_curriculum_assessment_analysis.png', dpi=300, bbox_inches='tight')
    print("✅ Curriculum assessment visualization saved as: stonegrove_curriculum_assessment_analysis.png")
    ...
    ax2.boxplot(grade_data, tick_labels=grade_order)
    ...
    ax4.boxplot(faculty_data, tick_labels=df['faculty'].unique())
    ...
    plt.tight_layout()
    plt.savefig('stonegrove_curriculum_assessment_detailed_analysis.png', dpi=300, bbox_inches='tight')
    print("✅ Detailed curriculum assessment analysis saved as: stonegrove_curriculum_assessment_detailed_analysis.png")
    
    # Create a third figure for module-specific analysis
    fig3, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
    
    # Top performing modules
    module_performance = df.groupby('module_title')['assessment_mark'].mean().sort_values(ascending=False)
    top_modules = module_performance.head(10)
    top_modules.plot(kind='barh', ax=ax1, color='lightgreen')
    ax1.set_title('Top 10 Performing Modules', fontsize=14, fontweight='bold')
    ax1.set_xlabel('Average Mark')
    
    # Lowest performing modules
    bottom_modules = module_performance.tail(10)
    bottom_modules.plot(kind='barh', ax=ax2, color='lightcoral')
    ax2.set_title('Lowest 10 Performing Modules', fontsize=14, fontweight='bold')
    ax2.set_xlabel('Average Mark')
    
    # Modules per student distribution
    modules_per_student = df.groupby('student_id').size()
    ax3.hist(modules_per_student, bins=20, alpha=0.7, color='skyblue', edgecolor='black')
    ax3.set_title('Distribution of Modules per Student', fontsize=14, fontweight='bold')
    ax3.set_xlabel('Number of Modules')
    ax3.set_ylabel('Number of Students')
    ax3.grid(True, alpha=0.3)
    
    # Performance by department
    dept_performance = df.groupby('department')['assessment_mark'].mean().sort_values(ascending=False)
    dept_performance.plot(kind='bar', ax=ax4, color='lightblue')
    ax4.set_title('Performance by Department', fontsize=14, fontweight='bold')
    ax4.set_ylabel('Average Mark')
    ax4.set_xticklabels(ax4.get_xticklabels(), rotation=45, ha='right')
    ax4.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('stonegrove_curriculum_module_analysis.png', dpi=300, bbox_inches='tight')
    print("✅ Module-specific analysis saved as: stonegrove_curriculum_module_analysis.png")
    ...
    # Module statistics
    print(f"\n📖 MODULE STATISTICS:")
    print(f"  • Total unique modules: {df['module_title'].nunique()}")
    print(f"  • Average modules per student: {len(df) / df['student_id'].nunique():.1f}")
    print(f"  • Most common module: {df['module_title'].mode()[0] if len(df['module_title'].mode()) > 0 else 'N/A'}")
    print(f"  • Highest performing module: {module_performance.index[0]} ({module_performance.iloc[0]:.1f})")
    print(f"  • Lowest performing module: {module_performance.index[-1]} ({module_performance.iloc[-1]:.1f})")

if __name__ == "__main__":
    visualize_curriculum_assessment_data() 