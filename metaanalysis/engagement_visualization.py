"""
Engagement visualization - comprehensive visualizations of engagement patterns.
Run from project root: python metaanalysis/engagement_visualization.py
"""
import pandas as pd
...
def create_engagement_visualizations():
    ...
    # Load engagement data (paths relative to project root)
    weekly_df = pd.read_csv('data/stonegrove_weekly_engagement.csv')
    semester_df = pd.read_csv('data/stonegrove_semester_engagement.csv')
    ...
    plt.tight_layout()
    plt.savefig('visualizations/stonegrove_engagement_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
