...
def run_year(
    ...
    from core_systems.enrolment_survey_system import EnrolmentSurveySystem
    ...
    return enrolled_df, progression_df, assessment_df, weekly_df, semester_df, graduate_outcomes_df, nss_df, enrolment_survey_df
...
def main():
    ...
    from core_systems.graduate_outcomes_system import GraduateOutcomesSystem
    from core_systems.nss_system import NSSSystem
    from core_systems.enrolment_survey_system import EnrolmentSurveySystem  # noqa: F401
    ...
    for i, acad_year in enumerate(ACADEMIC_YEARS):
        ...
        if enrolment_survey_df is not None and len(enrolment_survey_df) > 0:
            all_enrolment_survey.append(enrolment_survey_df)
        ...
        n_survey = len(enrolment_survey_df) if enrolment_survey_df is not None else 0
        year_elapsed = time.time() - year_start
        print(f"  Enrolled: {len(enrolled_df)}, Assessments: {len(assessment_df)}, Graduates: {n_grads}, NSS: {n_nss}, Survey: {n_survey}")
    ...
    if all_enrolment_survey:
        pd.concat(all_enrolment_survey, ignore_index=True).to_csv(
            data_dir / "stonegrove_enrolment_survey.csv", index=False
        )
        print(f"Saved stonegrove_enrolment_survey.csv")
